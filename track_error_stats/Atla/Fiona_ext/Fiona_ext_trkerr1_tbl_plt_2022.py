...
end_date = datetime.datetime(year=2022, month=9, day=25, hour=0)
...
#___
# Obtain the date/time and initial lat/lon from command line:
dirm = "/archive/u/dao_ops/f5294_fp/forecast/Y2022/M09/D20/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20220920_00+"
beg_date = datetime.datetime(year=2022, month=9, day=20, hour=0)
end_date = datetime.datetime(year=2022, month=9, day=25, hour=0)
...
#___
# Obtain the date/time and initial lat/lon from command line:
dirm = "/archive/u/dao_ops/f5294_fp/forecast/Y2022/M09/D21/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20220921_00+"
beg_date = datetime.datetime(year=2022, month=9, day=21, hour=0)
end_date = datetime.datetime(year=2022, month=9, day=25, hour=0)
...
#___
# Obtain the date/time and initial lat/lon from command line:
dirm = "/archive/u/dao_ops/f5294_fp/forecast/Y2022/M09/D22/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20220922_00+"
beg_date = datetime.datetime(year=2022, month=9, day=22, hour=0)
end_date = datetime.datetime(year=2022, month=9, day=25, hour=0)
...
beg_date = datetime.datetime(year=2022, month=9, day=19, hour=0)
end_date = datetime.datetime(year=2022, month=9, day=22, hour=12)
...
dft1 = pd.DataFrame((zip(trk_fcst_all,trk_err_all)), columns = ['FCST_HR','19/00Z'])
#print(dft1)
...
dft2 = pd.DataFrame((zip(trk_fcst_all1,trk_err_all1)), columns = ['FCST_HR','20/00Z'])
#print(dft2)
#sys.exit()
...
dft3 = pd.DataFrame((zip(trk_fcst_all2,trk_err_all2)), columns = ['FCST_HR','21/00Z'])
...
dft4 = pd.DataFrame((zip(trk_fcst_all3,trk_err_all3)), columns = ['FCST_HR','22/00Z'])
...
summarydf = pd.concat([dft1,dft2['20/00Z'],dft3['21/00Z'],dft4['22/00Z']], axis=1)
print(summarydf)
#sys.exit()
#----------------------------------------
#STATS CALC
...
summarydf = pd.concat([dft1,dft2['20/00Z'],dft3['21/00Z'],dft4['22/00Z']], axis=1)
print(summarydf)
with open('summary_4allstats_Fiona_ext.txt', 'w') as f:
    f.write(summarydf.to_string())

summarydf['mean'] = summarydf[['19/00Z','20/00Z','21/00Z','22/00Z']].mean(axis=1)
with open('summary_4onestats_Fiona_ext.txt', 'w') as f:
    f.write(summarydf.to_string())
summarydf['stddev'] = summarydf[['19/00Z','20/00Z','21/00Z','22/00Z']].std(axis=1)
summarydf['count'] = summarydf[['19/00Z','20/00Z','21/00Z','22/00Z']].count(axis=1)
