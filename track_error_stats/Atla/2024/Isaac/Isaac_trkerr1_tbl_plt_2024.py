...
end_date = datetime.datetime(year=2024, month=9, day=30, hour=12)
...
#list if lat >=dely ....
        ...
        lon_slice1 = lon_slice1+.85
        lon_slice2 = lon_slice2+.85
...
#list if lat >=dely ....
        ...
        lat_slice2 = lat_slice2+0.4
        lon_slice1 = lon_slice1+1.3
        lon_slice2 = lon_slice2+1.3
...
#___
# Obtain the date/time and initial lat/lon from command line:
dirm = "/home/gmao_ops/f5295_fp/run/.../archive/forecast/Y2024/M09/D28/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20240928_00+"
beg_date = datetime.datetime(year=2024, month=9, day=28, hour=0)
end_date = datetime.datetime(year=2024, month=9, day=30, hour=12)
...
#___
# Obtain the date/time and initial lat/lon from command line:
dirm = "/home/gmao_ops/f5295_fp/run/.../archive/forecast/Y2024/M09/D29/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20240929_00+"
beg_date = datetime.datetime(year=2024, month=9, day=29, hour=0)
end_date = datetime.datetime(year=2024, month=9, day=30, hour=12)
...
dft1 = pd.DataFrame((zip(trk_fcst_all1,trk_err_all1)), columns = ['FCST_HR','26/00Z'])
#print(dft2)
#sys.exit()
...
dft2 = pd.DataFrame((zip(trk_fcst_all2,trk_err_all2)), columns = ['FCST_HR','27/00Z'])
...
dft3 = pd.DataFrame((zip(trk_fcst_all3,trk_err_all3)), columns = ['FCST_HR','28/00Z'])
...
dft4 = pd.DataFrame((zip(trk_fcst_all4,trk_err_all4)), columns = ['FCST_HR','29/00Z'])

#---------------------#SUMMARY#----------------------
summarydf = pd.concat([dft1['FCST_HR'],dft1['26/00Z'],dft2['27/00Z'],dft3['28/00Z'],dft4['29/00Z']], axis=1)
print(summarydf)
#sys.exit()
#----------------------------------------
#STAS CALC
...
summarydf = pd.concat([dft1,dft2['27/00Z'],dft3['28/00Z'],dft4['29/00Z']], axis=1)
print(summarydf)
with open('summary_4allstats_Isaac.txt', 'w') as f:
    f.write(summarydf.to_string())
summarydf['mean'] = summarydf[['26/00Z','27/00Z','28/00Z','29/00Z']].mean(axis=1)
with open('summary_4onestats_Isaac.txt', 'w') as f:
    f.write(summarydf.to_string())
summarydf['stddev'] = summarydf[['26/00Z','27/00Z','28/00Z','29/00Z']].std(axis=1)
summarydf['count'] = summarydf[['26/00Z','27/00Z','28/00Z','29/00Z']].count(axis=1)
