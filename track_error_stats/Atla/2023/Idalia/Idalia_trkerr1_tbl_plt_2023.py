...
#___
# Obtain the date/time and initial lat/lon from command line:
dirm = "/home/gmao_ops/f5295_fp/run/.../archive/forecast/Y2023/M08/D30/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20230830_00+"
beg_date = datetime.datetime(year=2023, month=8, day=30, hour=0)
end_date = datetime.datetime(year=2023, month=8, day=30, hour=18)
...
summarydf = pd.concat([dft1,dft2['29/00Z'],dft3['30/00Z']], axis=1)
print(summarydf)
...
summarydf = pd.concat([dft1,dft2['29/00Z'],dft3['30/00Z']], axis=1)
print(summarydf)
with open('summary_4allstats_Idelia.txt', 'w') as f:
    f.write(summarydf.to_string())

summarydf['mean'] = summarydf[['28/00Z','29/00Z','30/00Z']].mean(axis=1)
with open('summary_4onestats_Idelia.txt', 'w') as f:
    f.write(summarydf.to_string())
summarydf['stddev'] = summarydf[['28/00Z','29/00Z','30/00Z']].std(axis=1)
summarydf['count'] = summarydf[['28/00Z','29/00Z','30/00Z']].count(axis=1)
