...
end_date = datetime.datetime(year=2023, month=9, day=18, hour=0)
...
dft1 = pd.DataFrame((zip(trk_fcst_all,trk_err_all)), columns = ['FCST_HR','10/00Z'])
#print(dft1)
...
dft2 = pd.DataFrame((zip(trk_fcst_all1,trk_err_all1)), columns = ['FCST_HR','11/00Z'])
#print(dft2)
#sys.exit()
...
dft3 = pd.DataFrame((zip(trk_fcst_all2,trk_err_all2)), columns = ['FCST_HR','12/00Z'])
...
dft4 = pd.DataFrame((zip(trk_fcst_all3,trk_err_all3)), columns = ['FCST_HR','13/00Z'])

summarydf = pd.concat([dft1,dft2['11/00Z'],dft3['12/00Z'],dft4['13/00Z']], axis=1)
print(summarydf)
...
summarydf = pd.concat([dft1,dft2['11/00Z'],dft3['12/00Z'],dft4['13/00Z']], axis=1)
print(summarydf)
with open('summary_4allstats_Margot.txt', 'w') as f:
    f.write(summarydf.to_string())

summarydf['mean'] = summarydf[['10/00Z','11/00Z','12/00Z','13/00Z']].mean(axis=1)
with open('summary_4onestats_Margot.txt', 'w') as f:
    f.write(summarydf.to_string())
summarydf['stddev'] = summarydf[['10/00Z','11/00Z','12/00Z','13/00Z']].std(axis=1)
summarydf['count'] = summarydf[['10/00Z','11/00Z','12/00Z','13/00Z']].count(axis=1)
