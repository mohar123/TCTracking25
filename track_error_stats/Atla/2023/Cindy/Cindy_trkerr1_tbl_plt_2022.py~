...
dirm = "/home/gmao_ops/f5295_fp/run/.../archive/forecast/Y2023/M06/D23/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20230623_00+"
beg_date = datetime.datetime(year=2023, month=6, day=23, hour=6)
end_date = datetime.datetime(year=2023, month=6, day=26, hour=6)
...
#___
# Obtain the date/time and initial lat/lon from command line:
dirm = "/home/gmao_ops/f5295_fp/run/.../archive/forecast/Y2023/M06/D24/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20230624_00+"
beg_date = datetime.datetime(year=2023, month=6, day=24, hour=0)
end_date = datetime.datetime(year=2023, month=6, day=26, hour=6)
...
#___
# Obtain the date/time and initial lat/lon from command line:
dirm = "/home/gmao_ops/f5295_fp/run/.../archive/forecast/Y2023/M06/D25/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20230625_00+"
beg_date = datetime.datetime(year=2023, month=6, day=25, hour=0)
end_date = datetime.datetime(year=2023, month=6, day=26, hour=6)
...
#___
# Obtain the date/time and initial lat/lon from command line:
dirm = "/home/gmao_ops/f5295_fp/run/.../archive/forecast/Y2023/M06/D26/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20230626_00+"
beg_date = datetime.datetime(year=2023, month=6, day=26, hour=0)
end_date = datetime.datetime(year=2023, month=6, day=26, hour=6)
...
dft1 = pd.DataFrame((zip(trk_fcst_all,trk_err_all)), columns = ['FCST_HR','23/00Z'])
#print(dft1)
...
dft2 = pd.DataFrame((zip(trk_fcst_all1,trk_err_all1)), columns = ['FCST_HR','24/00Z'])
#print(dft2)
#sys.exit()
...
dft3 = pd.DataFrame((zip(trk_fcst_all2,trk_err_all2)), columns = ['FCST_HR','25/00Z'])
...
dft4 = pd.DataFrame((zip(trk_fcst_all3,trk_err_all3)), columns = ['FCST_HR','26/00Z'])

#summary_df = pd.DataFrame((zip(trk_fcst_all,trk_err_all,trk_err_all1,trk_err_all2,trk_err_all3)), columns = ['FCST_HR','28/00Z','29/00Z','30/00Z','31/00Z'])
summarydf = pd.concat([dft1,dft2['24/00Z'],dft3['25/00Z'],dft4['26/00Z']], axis=1)
print(summarydf)
#sys.exit()
#----------------------------------------
#PLOTTING
#----------------------------------------
wlon,elon,nlat,slat = -65,-40.,10.,24.
...
ax.set_title('TC Track and Track errors (Nautical Miles): Cindy 2023')

p1, = ax.plot(lon_obs,lat_obs, label="TC Vitals",transform=data_transform,color="black")
p2, = ax.plot(lon_all,lat_all, label="Fcst Init: 23/00Z",transform=data_transform,color="green")
p3, = ax.plot(lon_all1,lat_all1, label="Fcst Init: 24/00Z",transform=data_transform,color="blue")
p4, = ax.plot(lon_all2,lat_all2, label="Fcst Init: 25/00Z",transform=data_transform,color="skyblue")
p5, = ax.plot(lon_all3,lat_all3, label="Fcst Init: 26/00Z",transform=data_transform,color="royalblue")

ax.legend(handles=[p1,p2,p3,p4,p5],prop={"size":6},ncol=2)
...
summarydf.update(summarydf[['FCST_HR','23/00Z','24/00Z','25/00Z','26/00Z']].applymap('{:,.0f}'.format))
thetable = plt.table(cellText=summarydf.values,colWidths = [0.1]*5,colLabels=("FCST_HR","23/00Z","24/00Z","25/00Z","26/00Z"),cellLoc = 'center', rowLoc = 'center',loc='bottom', bbox=[0.25, -1.52, 0.5, 1.35])
plt.subplots_adjust(left=0.2, bottom=0.5)
#thetable.auto_set_font_size(False)
#thetable.set_fontsize(12)
thetable.scale(1,4)
## change cell properties
plt.savefig('TCCindy_table_2023_01.png')
plt.show()
