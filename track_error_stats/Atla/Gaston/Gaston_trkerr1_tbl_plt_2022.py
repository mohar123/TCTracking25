...
beg_date = datetime.datetime(year=2022, month=9, day=22, hour=0)
end_date = datetime.datetime(year=2022, month=9, day=26, hour=12)
...
dft1 = pd.DataFrame((zip(trk_fcst_all,trk_err_all)), columns = ['FCST_HR','22/00Z'])
#print(dft1)
...
dft2 = pd.DataFrame((zip(trk_fcst_all1,trk_err_all1)), columns = ['FCST_HR','23/00Z'])
#print(dft2)
#sys.exit()
...
dft3 = pd.DataFrame((zip(trk_fcst_all2,trk_err_all2)), columns = ['FCST_HR','24/00Z'])
...
dft4 = pd.DataFrame((zip(trk_fcst_all3,trk_err_all3)), columns = ['FCST_HR','25/00Z'])
...
summarydf = pd.concat([dft1,dft2['23/00Z'],dft3['24/00Z'],dft4['25/00Z']], axis=1)
print(summarydf)
#sys.exit()
#----------------------------------------
#STATS CALC
...
summarydf = pd.concat([dft1,dft2['23/00Z'],dft3['24/00Z'],dft4['25/00Z']], axis=1)
print(summarydf)
with open('summary_4allstats_Gaston.txt', 'w') as f:
    f.write(summarydf.to_string())

summarydf['mean'] = summarydf[['22/00Z','23/00Z','24/00Z','25/00Z']].mean(axis=1)
with open('summary_4onestats_Gaston.txt', 'w') as f:
    f.write(summarydf.to_string())
summarydf['stddev'] = summarydf[['22/00Z','23/00Z','24/00Z','25/00Z']].std(axis=1)
summarydf['count'] = summarydf[['22/00Z','23/00Z','24/00Z','25/00Z']].count(axis=1)
