...
#----
# Obtain the date/time and initial lat/lon from command line:
dirm = "/archive/u/dao_ops/f5294_fp/forecast/Y2022/M07/D20/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20220720_00+"
beg_date = datetime.datetime(year=2022, month=7, day=20, hour=00)
end_date = datetime.datetime(year=2022, month=7, day=22, hour=00)
...
beg_date = datetime.datetime(year=2022, month=7, day=16, hour=6)
end_date = datetime.datetime(year=2022, month=7, day=20, hour=6)
...
dft1 = pd.DataFrame((zip(trk_fcst_all,trk_err_all)), columns = ['FCST_HR','16/00Z'])
#print(dft1)
...
dft2 = pd.DataFrame((zip(trk_fcst_all1,trk_err_all1)), columns = ['FCST_HR','17/00Z'])
...
dft3 = pd.DataFrame((zip(trk_fcst_all2,trk_err_all2)), columns = ['FCST_HR','18/00Z'])
#print(dft3)
...
dft4 = pd.DataFrame((zip(trk_fcst_all3,trk_err_all3)), columns = ['FCST_HR','19/00Z'])
...
dft5 = pd.DataFrame((zip(trk_fcst_all4,trk_err_all4)), columns = ['FCST_HR','20/00Z'])

summarydf = pd.concat([dft1,dft2['17/00Z'],dft3['18/00Z'],dft4['19/00Z'],dft5['20/00Z']], axis=1)
print(summarydf)
with open('summary_4allstats_Estelle.txt', 'w') as f:
    f.write(summarydf.to_string())

summarydf['mean'] = summarydf[['16/00Z','17/00Z','18/00Z','19/00Z','20/00Z']].mean(axis=1)
with open('summary_4onestats_Estelle.txt', 'w') as f:
    f.write(summarydf.to_string())
summarydf['stddev'] = summarydf[['16/00Z','17/00Z','18/00Z','19/00Z','20/00Z']].std(axis=1)
summarydf['count'] = summarydf[['16/00Z','17/00Z','18/00Z','19/00Z','20/00Z']].count(axis=1)
