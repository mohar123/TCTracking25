...
dirm = "/archive/u/dao_ops/f5294_fp/forecast/Y2022/M07/D16/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20220716_00+"
beg_date = datetime.datetime(year=2022, month=7, day=16, hour=6)
end_date = datetime.datetime(year=2022, month=7, day=21, hour=00)
...
for file in list_files:
    with xr.open_dataset(file, engine='netcdf4') as f:  
        ...
        slp_1 = f.SLP.sel(lat=slice(13.0,19.),lon=slice(-116.0, -103.))
...
#------------------------------
# Obtain the date/time and initial lat/lon from command line:
dirm = "/archive/u/dao_ops/f5294_fp/forecast/Y2022/M07/D17/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20220717_00+"
beg_date = datetime.datetime(year=2022, month=7, day=17, hour=00)
end_date = datetime.datetime(year=2022, month=7, day=22, hour=00)
...
print(list_files)
lon_all1, lat_all1, slp_min_all1, tim_all1 = [], [], [], []
del_lat = 1.0
del_lon = 2.0
...
for file in list_files:
    with xr.open_dataset(file, engine='netcdf4') as f:  
        ...
        slp_1 = f.SLP.sel(lat=slice(14.0,19.),lon=slice(-117.0, -104.0))
        slp_1_min = float(slp_1.min())
        loc1 = np.where(slp_1[0,:,:] == np.amin(slp_1[0,:,:]))
        lat1 = float(slp_1.lat[loc1[0]].values)
        lon1 = float(slp_1.lon[loc1[1]].values)
...
#print(tim_all,lat_all,lon_all,slp_min_all)
df2 = pd.DataFrame((zip(tim_all1,lat_all1,lon_all1,slp_min_all1)), columns = ['c1','c2','c3','c4'])
print(df2)


#------------------------------
# Obtain the date/time and initial lat/lon from command line:
dirm = "/archive/u/dao_ops/f5294_fp/forecast/Y2022/M07/D18/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20220718_00+"
beg_date = datetime.datetime(year=2022, month=7, day=18, hour=00)
end_date = datetime.datetime(year=2022, month=7, day=22, hour=00)
...
print(list_files)
lon_all2, lat_all2, slp_min_all2, tim_all2 = [], [], [], []
del_lat = 1.0
del_lon = 2.0
...
for file in list_files:
    with xr.open_dataset(file, engine='netcdf4') as f:  
        ...
        slp_1 = f.SLP.sel(lat=slice(14.0,19.8),lon=slice(-117.0, -107.))
        slp_1_min = float(slp_1.min())
        loc1 = np.where(slp_1[0,:,:] == np.amin(slp_1[0,:,:]))
        lat1 = float(slp_1.lat[loc1[0]].values)
        lon1 = float(slp_1.lon[loc1[1]].values)
...
#print(tim_all,lat_all,lon_all,slp_min_all)
df3 = pd.DataFrame((zip(tim_all2,lat_all2,lon_all2,slp_min_all2)), columns = ['c1','c2','c3','c4'])
print(df3)
#------------------------------
# Obtain the date/time and initial lat/lon from command line:
dirm = "/archive/u/dao_ops/f5294_fp/forecast/Y2022/M07/D19/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20220719_00+"
beg_date = datetime.datetime(year=2022, month=7, day=19, hour=00)
end_date = datetime.datetime(year=2022, month=7, day=22, hour=00)
...
print(list_files)
lon_all3, lat_all3, slp_min_all3, tim_all3 = [], [], [], []
del_lat = 1.0
del_lon = 2.0
...
for file in list_files:
    with xr.open_dataset(file, engine='netcdf4') as f:  
        ...
        slp_1 = f.SLP.sel(lat=slice(14.0,19.8),lon=slice(-117.0, -104.))
        slp_1_min = float(slp_1.min())
        loc1 = np.where(slp_1[0,:,:] == np.amin(slp_1[0,:,:]))
        lat1 = float(slp_1.lat[loc1[0]].values)
        lon1 = float(slp_1.lon[loc1[1]].values)
...
#print(tim_all,lat_all,lon_all,slp_min_all)
df4 = pd.DataFrame((zip(tim_all3,lat_all3,lon_all3,slp_min_all3)), columns = ['c1','c2','c3','c4'])
print(df4)
#------------------------------
diro = "/nfs3m/archive/sfa_cache08/projects/input/dao_ops/ops/flk/tcvitals/text/TCVITALS/Y2022/M07/"
colo = ".syndata.tcvitals"

beg_date = datetime.datetime(year=2022, month=7, day=16, hour=6)
end_date = datetime.datetime(year=2022, month=7, day=19, hour=18)
...
dft1 = pd.DataFrame((zip(tim_all,lat_all,lon_all,slp_min_all,trk_err_all)), columns = ['c1','c2','c3','c4','c5'])
print(dft1)

trk_err_all1 = []
for i in range(0,len(lat_obs)-3):
    print(tim_obs[i+3])
    d = datetime.datetime.strptime(tim_obs[i+3],"%Y%m%d%H")
    print(d.strftime("%Y-%m-%d-%H:%M:%S"))
    if (tim_obs[i+3] == tim_all1[i]):
       print(distance(lat_all1[i], lat_obs[i+3], lon_all1[i], lon_obs[i+3]))
       err = distance(lat_all1[i], lat_obs[i+3], lon_all1[i], lon_obs[i+3])
       trk_err_all1.append(err)

print(trk_err_all1)

dft2 = pd.DataFrame((zip(tim_all1,lat_all1,lon_all1,slp_min_all1,trk_err_all1)), columns = ['c1','c2','c3','c4','c5'])
print(dft2)

trk_err_all2 = []
for i in range(0,len(lat_obs)-7):
    print(tim_obs[i+7])
    d = datetime.datetime.strptime(tim_obs[i+7],"%Y%m%d%H")
    print(d.strftime("%Y-%m-%d-%H:%M:%S"))
    if (tim_obs[i+7] == tim_all2[i]):
        print(distance(lat_all2[i], lat_obs[i+7], lon_all2[i], lon_obs[i+7]))
        err = distance(lat_all2[i], lat_obs[i+7], lon_all2[i], lon_obs[i+7])
        trk_err_all2.append(err)

print(trk_err_all2)

dft3 = pd.DataFrame((zip(tim_all2,lat_all2,lon_all2,slp_min_all2,trk_err_all2)), columns = ['c1','c2','c3','c4','c5'])
print(df2)

trk_err_all3 = []
for i in range(0,len(lat_obs)-11):
    print(tim_obs[i+11])
    d = datetime.datetime.strptime(tim_obs[i+11],"%Y%m%d%H")
    print(d.strftime("%Y-%m-%d-%H:%M:%S"))
    if (tim_obs[i+11] == tim_all3[i]):
        print(distance(lat_all3[i], lat_obs[i+11], lon_all3[i], lon_obs[i+11]))
        err = distance(lat_all3[i], lat_obs[i+11], lon_all3[i], lon_obs[i+11])
        trk_err_all3.append(err)

print(trk_err_all3)

dft4 = pd.DataFrame((zip(tim_all3,lat_all3,lon_all3,slp_min_all3,trk_err_all3)), columns = ['c1','c2','c3','c4','c5'])
print(df3)
#----------------------------------------
wlon,elon,nlat,slat = -118.0,-102,20,12.5
...
ax.plot([lon_all2], [lat_all2], 'r*', transform=data_transform,color="cyan", markersize=8)
ax.plot([lon_all3], [lat_all3], 'r*', transform=data_transform,color="teal", markersize=8)
...
for i,j,k in zip(lat_all1,lon_all1,trk_err_all1):
    ax.annotate('{0:.0f}'.format(k),xy=(j,i),xytext=(j+0.2,i),color="blue",fontsize=8,horizontalalignment='left', verticalalignment='top')

for i,j,k in zip(lat_all2,lon_all2,trk_err_all2):
    ax.annotate('{0:.0f}'.format(k),xy=(j,i),xytext=(j+0.2,i),color="cyan",fontsize=8,horizontalalignment='left', verticalalignment='top')

for i,j,k in zip(lat_all3,lon_all3,trk_err_all3):
    ax.annotate('{0:.0f}'.format(k),xy=(j,i),xytext=(j+0.2,i),color="teal",fontsize=8,horizontalalignment='left', verticalalignment='top')

ax.set_title('TC Track and Track errors (km): Estelle 2022')

p1, = ax.plot(lon_obs,lat_obs, label="TC Vitals",transform=data_transform,color="black")
p2, = ax.plot(lon_all,lat_all, label="Fcst Init: 16/00Z",transform=data_transform,color="green")
p3, = ax.plot(lon_all1,lat_all1, label="Fcst Init: 17/00Z",transform=data_transform,color="blue")
p4, = ax.plot(lon_all2,lat_all2, label="Fcst Init: 18/00Z",transform=data_transform,color="cyan")
p5, = ax.plot(lon_all3,lat_all3, label="Fcst Init: 19/00Z",transform=data_transform,color="teal")
#ax.plot(lon_a06,lat_a06, transform=data_transform,color="blue")

ax.legend(handles=[p1,p2,p3,p4,p5],prop={"size":10})
...
plt.savefig('TCEstelle_2022_4.png')
plt.show()
