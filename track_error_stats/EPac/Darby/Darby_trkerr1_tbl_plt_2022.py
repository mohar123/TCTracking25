...
for file in list_files:
    with xr.open_dataset(file, engine='netcdf4') as f:  
        ...
        slp_1 = f.SLP.sel(lat=slice(14.0,17.),lon=slice(-142.0, -115.5))
...
end_date = datetime.datetime(year=2022, month=7, day=17, hour=00)
...
for file in list_files:
    with xr.open_dataset(file, engine='netcdf4') as f:  
        ...
        slp_1 = f.SLP.sel(lat=slice(14.0,17.5),lon=slice(-146.0, -124.5))
        slp_1_min = float(slp_1.min())
        loc1 = np.where(slp_1[0,:,:] == np.amin(slp_1[0,:,:]))
        lat1 = float(slp_1.lat[loc1[0]].values)
        lon1 = float(slp_1.lon[loc1[1]].values)
...
#------------------------------
# Obtain the date/time and initial lat/lon from command line:
dirm = "/archive/u/dao_ops/f5294_fp/forecast/Y2022/M07/D13/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20220713_00+"
beg_date = datetime.datetime(year=2022, month=7, day=13, hour=00)
end_date = datetime.datetime(year=2022, month=7, day=16, hour=00)
...
for file in list_files:
    with xr.open_dataset(file, engine='netcdf4') as f:  
        ...
        slp_1 = f.SLP.sel(lat=slice(14.0,17.),lon=slice(-147.0, -130.))
        slp_1_min = float(slp_1.min())
        loc1 = np.where(slp_1[0,:,:] == np.amin(slp_1[0,:,:]))
        lat1 = float(slp_1.lat[loc1[0]].values)
        lon1 = float(slp_1.lon[loc1[1]].values)
...
#---
# Obtain the date/time and initial lat/lon from command line:
dirm = "/archive/u/dao_ops/f5294_fp/forecast/Y2022/M07/D14/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20220714_00+"
beg_date = datetime.datetime(year=2022, month=7, day=14, hour=00)
end_date = datetime.datetime(year=2022, month=7, day=16, hour=00)
...
for file in list_files:
    with xr.open_dataset(file, engine='netcdf4') as f:  
        ...
        slp_1 = f.SLP.sel(lat=slice(14.0,17.),lon=slice(-147, -130.))
        slp_1_min = float(slp_1.min())
        loc1 = np.where(slp_1[0,:,:] == np.amin(slp_1[0,:,:]))
        lat1 = float(slp_1.lat[loc1[0]].values)
        lon1 = float(slp_1.lon[loc1[1]].values)
...
beg_date = datetime.datetime(year=2022, month=7, day=11, hour=0)
end_date = datetime.datetime(year=2022, month=7, day=15, hour=0)
...
dft1 = pd.DataFrame((zip(trk_fcst_all,trk_err_all)), columns = ['FCST_HR','11/00Z'])
#print(dft1)
...
dft2 = pd.DataFrame((zip(trk_fcst_all1,trk_err_all1)), columns = ['FCST_HR','12/00Z'])
...
dft3 = pd.DataFrame((zip(trk_fcst_all2,trk_err_all2)), columns = ['FCST_HR','13/00Z'])
#print(dft3)
...
dft4 = pd.DataFrame((zip(trk_fcst_all3,trk_err_all3)), columns = ['FCST_HR','14/00Z'])

summarydf = pd.concat([dft1,dft2['12/00Z'],dft3['13/00Z'],dft4['14/00Z']], axis=1)
print(summarydf)
with open('summary_4allstats_Darby.txt', 'w') as f:
    f.write(summarydf.to_string())

summarydf['mean'] = summarydf[['11/00Z','12/00Z','13/00Z','14/00Z']].mean(axis=1)
with open('summary_4onestats_Darby.txt', 'w') as f:
    f.write(summarydf.to_string())
summarydf['stddev'] = summarydf[['11/00Z','12/00Z','13/00Z','14/00Z']].std(axis=1)
summarydf['count'] = summarydf[['11/00Z','12/00Z','13/00Z','14/00Z']].count(axis=1)
