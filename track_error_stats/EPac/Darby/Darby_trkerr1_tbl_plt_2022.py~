...
dirm = "/archive/u/dao_ops/f5294_fp/forecast/Y2022/M07/D10/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20220710_00+"
beg_date = datetime.datetime(year=2022, month=7, day=10, hour=00)
end_date = datetime.datetime(year=2022, month=7, day=15, hour=00)
...
for file in list_files:
    with xr.open_dataset(file, engine='netcdf4') as f:  
        ...
        slp_1 = f.SLP.sel(lat=slice(14.0,16.),lon=slice(-133.0, -113.5))
...
#------------------------------
# Obtain the date/time and initial lat/lon from command line:
dirm = "/archive/u/dao_ops/f5294_fp/forecast/Y2022/M07/D11/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20220711_00+"
beg_date = datetime.datetime(year=2022, month=7, day=11, hour=00)
end_date = datetime.datetime(year=2022, month=7, day=16, hour=00)
...
#------------------------------
# Obtain the date/time and initial lat/lon from command line:
dirm = "/archive/u/dao_ops/f5294_fp/forecast/Y2022/M07/D12/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20220712_00+"
beg_date = datetime.datetime(year=2022, month=7, day=12, hour=00)
end_date = datetime.datetime(year=2022, month=7, day=16, hour=00)
...
for file in list_files:
    with xr.open_dataset(file, engine='netcdf4') as f:  
        ...
        slp_1 = f.SLP.sel(lat=slice(14.0,16.),lon=slice(-134.0, -113.5))
        slp_1_min = float(slp_1.min())
        loc1 = np.where(slp_1[0,:,:] == np.amin(slp_1[0,:,:]))
        lat1 = float(slp_1.lat[loc1[0]].values)
        lon1 = float(slp_1.lon[loc1[1]].values)
...
beg_date = datetime.datetime(year=2022, month=7, day=10, hour=0)
end_date = datetime.datetime(year=2022, month=7, day=13, hour=0)
...
trk_err_all1 = []
for i in range(0,len(lat_obs)-4):
    print(tim_obs[i+4])
    d = datetime.datetime.strptime(tim_obs[i+4],"%Y%m%d%H")
    print(d.strftime("%Y-%m-%d-%H:%M:%S"))
    if (tim_obs[i+4] == tim_all1[i]):
       print(distance(lat_all1[i], lat_obs[i+4], lon_all1[i], lon_obs[i+4]))
       err = distance(lat_all1[i], lat_obs[i+4], lon_all1[i], lon_obs[i+4])
...
trk_err_all2 = []
for i in range(0,len(lat_obs)-8):
    print(tim_obs[i+8])
    d = datetime.datetime.strptime(tim_obs[i+8],"%Y%m%d%H")
    print(d.strftime("%Y-%m-%d-%H:%M:%S"))
    if (tim_obs[i+8] == tim_all2[i]):
       print(distance(lat_all2[i], lat_obs[i+8], lon_all2[i], lon_obs[i+8]))
       err = distance(lat_all2[i], lat_obs[i+8], lon_all2[i], lon_obs[i+8])
...
print(df2)
#----------------------------------------
wlon,elon,nlat,slat = -134.0,-113,20,13
...
ax.set_title('TC Track and Track errors (km): Darby 2022')

p1, = ax.plot(lon_obs,lat_obs, label="TC Vitals",transform=data_transform,color="black")
p2, = ax.plot(lon_all,lat_all, label="Fcst Init: 10/00Z",transform=data_transform,color="green")
p3, = ax.plot(lon_all1,lat_all1, label="Fcst Init: 11/00Z",transform=data_transform,color="blue")
p4, = ax.plot(lon_all2,lat_all2, label="Fcst Init: 12/00Z",transform=data_transform,color="cyan")
#ax.plot(lon_a06,lat_a06, transform=data_transform,color="blue")

ax.legend(handles=[p1,p2,p3,p4],prop={"size":10})
...
plt.savefig('TCDarby_2022_3.png')
plt.show()
