...
end_date = datetime.datetime(year=2023, month=8, day=17, hour=12)
...
summarydf = pd.concat([dft1,dft2['16/00Z'],dft3['17/00Z']], axis=1)
print(summarydf)
...
with open('summary_4allstats_Greg.txt', 'w') as f:
    f.write(summarydf.to_string())

summarydf['mean'] = summarydf[['15/00Z','16/00Z','17/00Z']].mean(axis=1)
with open('summary_4onestats_Greg.txt', 'w') as f:
    f.write(summarydf.to_string())
summarydf['stddev'] = summarydf[['15/00Z','16/00Z','17/00Z']].std(axis=1)
summarydf['count'] = summarydf[['15/00Z','16/00Z','17/00Z']].count(axis=1)
