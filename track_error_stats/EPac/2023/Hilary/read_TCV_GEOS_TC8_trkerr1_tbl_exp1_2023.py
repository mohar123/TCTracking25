...
dirm = "/home/gmao_ops/f5295_fp/run/.../archive/forecast/Y2023/M08/D17/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20230817_00+"
beg_date = datetime.datetime(year=2023, month=8, day=17, hour=0)
end_date = datetime.datetime(year=2023, month=8, day=21, hour=0)
...
#___
# Obtain the date/time and initial lat/lon from command line:
dirm = "/home/gmao_ops/f5295_fp/run/.../archive/forecast/Y2023/M08/D18/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20230818_00+"
beg_date = datetime.datetime(year=2023, month=8, day=18, hour=0)
end_date = datetime.datetime(year=2023, month=8, day=21, hour=0)
...
#___
# Obtain the date/time and initial lat/lon from command line:
dirm = "/home/gmao_ops/f5295_fp/run/.../archive/forecast/Y2023/M08/D19/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20230819_00+"
beg_date = datetime.datetime(year=2023, month=8, day=19, hour=0)
end_date = datetime.datetime(year=2023, month=8, day=21, hour=6)
...
#___
# Obtain the date/time and initial lat/lon from command line:
dirm = "/home/gmao_ops/f5295_fp/run/.../archive/forecast/Y2023/M08/D20/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20230820_00+"
beg_date = datetime.datetime(year=2023, month=8, day=20, hour=0)
end_date = datetime.datetime(year=2023, month=8, day=21, hour=0)
...
dft1 = pd.DataFrame((zip(trk_fcst_all,trk_err_all)), columns = ['FCST_HR','17/00Z'])
#print(dft1)
...
dft2 = pd.DataFrame((zip(trk_fcst_all1,trk_err_all1)), columns = ['FCST_HR','18/00Z'])
#print(dft2)
#sys.exit()
...
dft3 = pd.DataFrame((zip(trk_fcst_all2,trk_err_all2)), columns = ['FCST_HR','19/00Z'])
...
dft4 = pd.DataFrame((zip(trk_fcst_all3,trk_err_all3)), columns = ['FCST_HR','20/00Z'])

summarydf = pd.concat([dft1,dft2['18/00Z'],dft3['19/00Z'],dft4['20/00Z']], axis=1)
print(summarydf)
...
#PLOTTING
#----------------------------------------
wlon,elon,nlat,slat = -100,-120.,9.,35.
...
ax.set_title('TC Track and Track errors (Nautical Miles): Hilary EPAC 2023')

p1, = ax.plot(lon_obs,lat_obs, label="TC Vitals",transform=data_transform,color="black")
p2, = ax.plot(lon_all,lat_all, label="Fcst Init: 17/00Z",transform=data_transform,color="green")
p3, = ax.plot(lon_all1,lat_all1, label="Fcst Init: 18/00Z",transform=data_transform,color="blue")
p4, = ax.plot(lon_all2,lat_all2, label="Fcst Init: 19/00Z",transform=data_transform,color="skyblue")
p5, = ax.plot(lon_all3,lat_all3, label="Fcst Init: 20/00Z",transform=data_transform,color="royalblue")
...
summarydf.update(summarydf[['FCST_HR','17/00Z','18/00Z','19/00Z','20/00Z']].applymap('{:,.0f}'.format))
thetable = plt.table(cellText=summarydf.values,colWidths = [0.1]*6,colLabels=("FCST_HR","17/00Z","18/00Z","19/00Z","20/00Z"),cellLoc = 'center', rowLoc = 'center',loc='bottom', bbox=[0.25, -1.55, 0.5, 1.37])
