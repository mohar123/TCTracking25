...
#_____________________________________
# Obtain the date/time and initial lat/lon from command line:
dirm = "/home/gmao_ops/f5295_fp/run/.../archive/forecast/Y2023/M08/D12/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20230812_00+"
beg_date = datetime.datetime(year=2023, month=8, day=12, hour=0)
end_date = datetime.datetime(year=2023, month=8, day=14, hour=6)
...
#print(tim_all,lat_all,lon_all,slp_min_all)
df10 = pd.DataFrame((zip(tim_all9,lat_all9,lon_all9,slp_min_all9,fcst_all9)), columns = ['c1','c2','c3','c4','c5'])
print(df10)
...
print(dfo)
#sys.exit()
#----------------------Special Case DORA Eatsren Hemisphere------------------
#
beg_date = datetime.datetime(year=2023, month=8, day=12, hour=6)
end_date = datetime.datetime(year=2023, month=8, day=14, hour=6)
...
lon_obsD, lat_obsD, slp_min_obsD, tim_obsD = [], [], [], []
while cur_date < end_date:
    print(cur_date)
    file = cur_date.strftime(diro+"M%m/gfs.%y%m%d.t%Hz"+colo)
    list_fileso.append(file)
...
#print(tim_obs,lat_obs,lon_obs,slp_min_obs)
dfoD = pd.DataFrame((zip(tim_obsD,lat_obsD,lon_obsD,slp_min_obsD)), columns = ['c1','c2','c3','c4'])
print(dfoD)
#
#----------------------------------------
...
dft10 = pd.DataFrame((zip(trk_fcst_all9,trk_err_all9)), columns = ['FCST_HR','12/00Z'])


#summarydf = pd.concat([dft1,dft2['03/00Z'],dft3['04/00Z'],dft4['05/00Z'],dft5['06/00Z']], axis=1)
summarydf = pd.concat([dft2['FCST_HR'],dft1['02/00Z'],dft2['03/00Z'],dft3['04/00Z'],dft4['05/00Z'],dft5['06/00Z'],dft6['07/00Z'],dft7['08/00Z'],dft8['09/00Z'],dft9['10/00Z'],dft10['12/00Z']], axis=1)
print(summarydf)
...
#PLOTTING
#----------------------------------------
#wlon,elon,nlat,slat = -105.,-190.,0.,30.
...
map_projection = ccrs.PlateCarree(central_longitude=-150)
...
minlon = -50 + cm
maxlon = +50 + cm
ax.set_extent([minlon, maxlon,(0),(30)], ccrs.PlateCarree())
...
#ax.plot([lon_all2], [lat_all2], 'r*', transform=data_transform,color="skyblue", markersize=8)
#ax.plot([lon_all3], [lat_all3], 'r*', transform=data_transform,color="royalblue", markersize=8)
#ax.plot([lon_all4], [lat_all4], 'r*', transform=data_transform,color="dodgerblue", markersize=8)
#ax.plot([lon_all5], [lat_all5], 'r*', transform=data_transform,color="skyblue", markersize=8,linestyle="--")
#ax.plot([lon_all6], [lat_all6], 'r*', transform=data_transform,color="royalblue", markersize=8,linestyle="--")
#ax.plot([lon_all7], [lat_all7], 'r*', transform=data_transform,color="dodgerblue", markersize=8,linestyle="--")
...
ax.set_title('TC Track and Track errors (Nautical Miles): Dora EPAC 2023')

#p1, = ax.plot(lon_obs,lat_obs, label="TC Vitals",transform=data_transform,color="black",marker="*")
p1, = ax.plot(lon_obs,lat_obs,"*k", label="TC Vitals",transform=data_transform)
p2, = ax.plot(lon_all,lat_all, label="Fcst Init: 02/00Z",transform=data_transform,color="green")
p3, = ax.plot(lon_all1,lat_all1, label="Fcst Init: 03/00Z",transform=data_transform,color="blue")
p4, = ax.plot(lon_all2,lat_all2, label="Fcst Init: 04/00Z",transform=data_transform,color="skyblue")
p5, = ax.plot(lon_all3,lat_all3, label="Fcst Init: 05/00Z",transform=data_transform,color="royalblue")
p6, = ax.plot(lon_all4,lat_all4, label="Fcst Init: 06/00Z", transform=data_transform,color="dodgerblue")
p7, = ax.plot(lon_all5,lat_all5, label="Fcst Init: 07/00Z", transform=data_transform,color="skyblue", markersize=8,linestyle="--")
p8, = ax.plot(lon_all6,lat_all6, label="Fcst Init: 08/00Z", transform=data_transform,color="royalblue", markersize=8,linestyle="--")
p9, = ax.plot(lon_all7,lat_all7, label="Fcst Init: 09/00Z", transform=data_transform,color="dodgerblue", markersize=8,linestyle="--")
p10, = ax.plot(lon_all8,lat_all8, label="Fcst Init: 10/00Z", transform=data_transform,color="green", markersize=12,linestyle=":")
p11, = ax.plot(lon_all9,lat_all9, label="Fcst Init: 12/00Z", transform=data_transform,color="blue", markersize=12,linestyle=":")
p12, = ax.plot(lon_obsD,lat_obsD,"*k", label="TC Vitals",transform=data_transform)

ax.legend(handles=[p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11],prop={"size":8},ncol=4,bbox_to_anchor=(.85, -0.15),fancybox=True)
#ax.legend(handles=[p1,p2,p3,p4,p5,p6],prop={"size":10})

#gl = ax.gridlines(draw_labels=True, crs=proj)
gl = ax.gridlines(draw_labels=True, crs=ccrs.PlateCarree(), xlocs=[185,160,-175,-150,-125,-100])
#ax.gridlines(draw_labels=True, crs=ccrs.PlateCarree(), xlocs=[185,160,-175,-150,-125,-100])
gl.xlabels_top = False
gl.ylabels_left = False
#gl.xformatter = LONGITUDE_FORMATTER
gl.yformatter = LATITUDE_FORMATTER

plt.savefig('TCDora_table_2023_03.png')
plt.show()
sys.exit()
#-------------
#TABLE
...
summarydf.update(summarydf[['FCST_HR','02/00Z','03/00Z','04/00Z','05/00Z','06/00Z','07/00Z','08/00Z','09/00Z']].applymap('{:,.0f}'.format))
thetable = plt.table(cellText=summarydf.values,colWidths = [0.7]*9,colLabels=("FCST_HR","02/00Z","03/00Z","04/00Z","05/00Z","06/00Z",'07/00Z','08/00Z','09/00Z'),cellLoc = 'center', rowLoc = 'center',loc='bottom', bbox=[0.0, -2.8, 1.0, 2.4])
plt.subplots_adjust(left=0.2, bottom=0.5)
#thetable.auto_set_font_size(False)
thetable.set_fontsize(12)
thetable.scale(2,8)
## change cell properties
plt.savefig('TCDora_table_2023_02.png')
plt.show()
