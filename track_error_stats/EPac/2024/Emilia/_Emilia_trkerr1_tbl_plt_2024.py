...
end_date = datetime.datetime(year=2024, month=8, day=8, hour=18)
...
#___
# Obtain the date/time and initial lat/lon from command line:
dirm = "/home/gmao_ops/f5295_fp/run/.../archive/forecast/Y2024/M08/D06/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20240806_00+"
beg_date = datetime.datetime(year=2024, month=8, day=6, hour=0)
end_date = datetime.datetime(year=2024, month=8, day=8, hour=18)
...
#___
# Obtain the date/time and initial lat/lon from command line:
dirm = "/home/gmao_ops/f5295_fp/run/.../archive/forecast/Y2024/M08/D07/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20240807_00+"
beg_date = datetime.datetime(year=2024, month=8, day=7, hour=0)
end_date = datetime.datetime(year=2024, month=8, day=8, hour=18)
...
#___
# Obtain the date/time and initial lat/lon from command line:
dirm = "/home/gmao_ops/f5295_fp/run/.../archive/forecast/Y2024/M08/D08/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20240808_00+"
beg_date = datetime.datetime(year=2024, month=8, day=8, hour=0)
end_date = datetime.datetime(year=2024, month=8, day=8, hour=18)
...
dft1 = pd.DataFrame((zip(trk_fcst_all1,trk_err_all1)), columns = ['FCST_HR','05/00Z'])
#print(dft2)
#sys.exit()
...
dft2 = pd.DataFrame((zip(trk_fcst_all2,trk_err_all2)), columns = ['FCST_HR','06/00Z'])
...
dft3 = pd.DataFrame((zip(trk_fcst_all3,trk_err_all3)), columns = ['FCST_HR','07/00Z'])
...
dft4 = pd.DataFrame((zip(trk_fcst_all4,trk_err_all4)), columns = ['FCST_HR','08/00Z'])

#---------------------#SUMMARY#----------------------
summarydf = pd.concat([dft1['FCST_HR'],dft1['05/00Z'],dft2['06/00Z'],dft3['07/00Z'],dft4['08/00Z']], axis=1)
print(summarydf)
#sys.exit()
#----------------------------------------
summarydf = pd.concat([dft1,dft2['06/00Z'],dft3['07/00Z'],dft4['08/00Z']], axis=1)
print(summarydf)
with open('summary_4allstats_Emilia.txt', 'w') as f:
    f.write(summarydf.to_string())

summarydf['mean'] = summarydf[['05/00Z','06/00Z','07/00Z','08/00Z']].mean(axis=1)
with open('summary_4onestats_Emilia.txt', 'w') as f:
    f.write(summarydf.to_string())
