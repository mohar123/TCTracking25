...
beg_date = datetime.datetime(year=2025, month=9, day=3, hour=00)
end_date = datetime.datetime(year=2025, month=9, day=5, hour=12)
...
dft1 = pd.DataFrame((zip(trk_fcst_all1,trk_err_all1)), columns = ['FCST_HR','03/00Z'])
#print(dft2)
#sys.exit()
...
dft2 = pd.DataFrame((zip(trk_fcst_all2,trk_err_all2)), columns = ['FCST_HR','04/00Z'])

#---------------------#SUMMARY#----------------------
summarydf = pd.concat([dft1['FCST_HR'],dft1['03/00Z'],dft2['04/00Z']], axis=1)
print(summarydf)
#sys.exit()
#----------------------------------------
#PLOTTING
#----------------------------------------
wlon,elon,nlat,slat = -107.,-117.,17.,26.
...
ax.set_title('TC Track Forecast: Lorena East Pacific 2025')

p1, = ax.plot(lon_obs,lat_obs, label="TC Vitals",transform=data_transform,color="black")
p2, = ax.plot(lon_all1,lat_all1, label="Fcst Init: 03/00Z",transform=data_transform,color="lightgreen")
p3, = ax.plot(lon_all2,lat_all2, label="Fcst Init: 04/00Z",transform=data_transform,color="lime")
...
plt.savefig('TCLorena_table_2025.png')
plt.show()
sys.exit()
#-------------
#TABLE
