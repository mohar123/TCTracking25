...
beg_date = datetime.datetime(year=2025, month=6, day=30, hour=00)
end_date = datetime.datetime(year=2025, month=7, day=3, hour=18)
...
dirm = "/home/gmao_ops/f5295_fp/run/.../archive/forecast/Y2025/M06/D30/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20250630_00+"
beg_date = datetime.datetime(year=2025, month=6, day=30, hour=0)
end_date = datetime.datetime(year=2025, month=7, day=3, hour=18)
...
#___
# Obtain the date/time and initial lat/lon from command line:
dirm = "/home/gmao_ops/f5295_fp/run/.../archive/forecast/Y2025/M07/D01/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20250701_00+"
beg_date = datetime.datetime(year=2025, month=7, day=1, hour=0)
end_date = datetime.datetime(year=2025, month=7, day=3, hour=18)
...
#___
# Obtain the date/time and initial lat/lon from command line:
dirm = "/home/gmao_ops/f5295_fp/run/.../archive/forecast/Y2025/M07/D02/H00/"
col = "GEOS.fp.fcst.inst3_3d_asm_Np.20250702_00+"
beg_date = datetime.datetime(year=2025, month=7, day=2, hour=0)
end_date = datetime.datetime(year=2025, month=7, day=3, hour=18)
...
dft1 = pd.DataFrame((zip(trk_fcst_all1,trk_err_all1)), columns = ['FCST_HR','30/00Z'])
#print(dft2)
#sys.exit()
...
dft2 = pd.DataFrame((zip(trk_fcst_all2,trk_err_all2)), columns = ['FCST_HR','01/00Z'])
...
dft3 = pd.DataFrame((zip(trk_fcst_all3,trk_err_all3)), columns = ['FCST_HR','02/00Z'])

#---------------------#SUMMARY#----------------------
summarydf = pd.concat([dft1['FCST_HR'],dft1['30/00Z'],dft2['01/00Z'],dft3['02/00Z']], axis=1)
print(summarydf)
#sys.exit()
#----------------------------------------
#PLOTTING
#----------------------------------------
wlon,elon,nlat,slat = -97.,-115.,10.,23.
...
ax.set_title('TC Track Forecast: Flossie East Pacific 2025')

p1, = ax.plot(lon_obs,lat_obs, label="TC Vitals",transform=data_transform,color="black")
p2, = ax.plot(lon_all1,lat_all1, label="Fcst Init: 30/00Z",transform=data_transform,color="lightgreen")
p3, = ax.plot(lon_all2,lat_all2, label="Fcst Init: 01/00Z",transform=data_transform,color="cyan")
p4, = ax.plot(lon_all3,lat_all3, label="Fcst Init: 02/00Z",transform=data_transform,color="blue")

#ax.legend(handles=[p1,p2,p3,p4,p5],prop={"size":6},ncol=2,bbox_to_anchor=(.85, -0.15),fancybox=True)
ax.legend(handles=[p1,p2,p3,p4],prop={"size":6},ncol=3)
...
plt.savefig('TCFlossie_table_2025.png')
plt.show()
sys.exit()
#-------------
#TABLE
